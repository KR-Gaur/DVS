...
<?php
	...
	session_start();
	if(!isset($_POST['DEL_CANDIDATE_ID']) 
		|| trim($_POST['DEL_CANDIDATE_ID'])== '')				            	//to handle empty request
	{
		header("refresh:0;url=delete_candidate.php");
			echo "is empty";
		
	}
	else
	{
	echo "<br> DEL_CANDIDATE_ID";
	echo $_POST['DEL_CANDIDATE_ID'];

	$_SESSION['DEL_CANDIDATE_ID']=$_POST['DEL_CANDIDATE_ID'];				

	echo $_SESSION['DEL_CANDIDATE_ID'];
				...
				//echo "<br><br><br>into main code";
					...
					{		
				       ...
				       // $_SESSION['RESIDENT_PROOF']=$row['RESIDENT_PROOF'];
						...
						header("refresh:5;url=delete_candidate_in_form.php");				//load delete_candidate_in_form.php after refresh
					}
				
				else
					{
						echo '<br>not found pls....!!';
						header("refresh:1;url=delete_candidate.php");		
